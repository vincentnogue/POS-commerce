...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      // tenant's connection_id and create an order — or issue a refund —
      // against that tenant's PayPal account. Found during a follow-up
      // security audit of every edge function after the stripe-payments
      // fix.
      ...
      if (action === "refund" && !["admin", "manager", "super_admin"].includes(callerMember.role)) {
        return new Response(
          JSON.stringify({ success: false, message: "Insufficient permission for a refund" }),
          { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }

      // RATE LIMIT: each order creation/refund is a real PayPal
      // transaction. 30/hour is generous for real checkout volume.
      const rlRes = await fetch(`${supabaseUrl}/rest/v1/rpc/check_rate_limit`, {
        method: "POST",
        headers: { apikey: serviceRoleKey, Authorization: `Bearer ${serviceRoleKey}`, "Content-Type": "application/json" },
        body: JSON.stringify({ p_tenant_id: body.tenant_id, p_function_name: "paypal-payments", p_max_calls: 30, p_window_minutes: 60 }),
      });
