...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      // tenant's connection_id and trigger an M-Pesa STK push (a real
      // payment prompt on a real customer's phone) under that tenant's
      // short code — fraud/harassment potential and reputational damage
      // to the target business, independent of any refund capability.
      // Found during a follow-up security audit of every edge function
      // after the stripe-payments fix.
      ...
      // RATE LIMIT: each STK push prompts a real customer's phone for a
      // real payment. A runaway loop here is a fraud/harassment vector,
      // not just a cost issue — 30/hour is generous for a busy store's
      // real checkout volume.
      const rlRes = await fetch(`${supabaseUrl}/rest/v1/rpc/check_rate_limit`, {
        method: "POST",
        headers: { apikey: serviceRoleKey, Authorization: `Bearer ${serviceRoleKey}`, "Content-Type": "application/json" },
        body: JSON.stringify({ p_tenant_id: body.tenant_id, p_function_name: "mpesa-payments", p_max_calls: 30, p_window_minutes: 60 }),
      });
