    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-3">
          <Logo clickable />
          <button onClick={onClose} className="rounded-full p-1 text-ink-500 hover:bg-brand-100 lg:hidden">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="px-4 pb-2">
          <div className="relative">
            <button
              ...
              className="flex w-full items-center justify-between rounded-xl border border-brand-100 bg-white px-3 py-2.5 text-left transition hover:border-brand-200"
            >
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-ink-900">{tenant?.name ?? 'Aucun magasin'}</p>
                <p className="truncate text-xs text-ink-500">
                  {tenant?.city ?? '—'} · {tenant?.country_name}
                </p>
              </div>
              <ChevronDown size={16} className={`text-ink-400 transition-transform ${tenantMenuOpen ? 'rotate-180' : ''}`} />
            </button>
            <AnimatePresence>
              {tenantMenuOpen && tenants.length > 0 && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -4 }}
                  className="absolute left-0 right-0 top-full z-10 mt-1 max-h-64 overflow-auto rounded-xl border border-ink-200 bg-white py-1 shadow-float scroll-thin"
                >
                  {tenants.map(({ tenant: t, member: m }) => (
                    ...
                    >
                      <div className="min-w-0">
                        <p className="truncate text-ink-900">{t.name}</p>
                        <p className="truncate text-xs text-ink-500">{t.city}</p>
                      </div>
                      <span className="ml-2 shrink-0 rounded-full bg-ink-100 px-2 py-0.5 text-[10px] uppercase text-ink-600">
                ...
                className={({ isActive }) =>
                  `group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors ${
                    isActive
                      ...
                      : 'text-ink-700 hover:bg-white/60 hover:text-brand-700'
                  }`
                }
              >
                <Icon size={18} strokeWidth={1.8} className="shrink-0 text-ink-500 group-hover:text-brand-600" />
        ...
        {/* Profile + sign out */}
        <div className="m-3 rounded-xl border border-brand-100 bg-white p-3">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold text-ink-900">{member?.display_name ?? tenant?.name ?? user?.email}</p>
              <p className="truncate text-xs text-ink-500">{member ? ROLE_LABELS[(member.role as Role) ?? 'staff'] : '—'}</p>
            </div>
          </div>
          <button
            onClick={handleSignOut}
            className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg border border-ink-200 bg-white px-3 py-2 text-sm font-semibold text-ink-700 transition hover:border-error-200 hover:bg-error-50 hover:text-error-600"
          >
            <LogOut size={15} /> Se déconnecter
          </button>
        </div>
        <div className="px-4 pb-4 text-center text-[10px] text-ink-400">
