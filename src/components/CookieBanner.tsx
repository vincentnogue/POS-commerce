    ...
    <AnimatePresence>
      {show && (
        <motion.div
          ...
          className="fixed bottom-4 left-1/2 z-50 w-[calc(100%-2rem)] max-w-xl -translate-x-1/2 rounded-2xl2 border border-ink-200 bg-white p-5 shadow-float"
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              {showPrefs && (
                <div className="mt-3 space-y-2 rounded-xl bg-ink-50 p-3">
                  <label className="flex items-center justify-between text-sm">
                    <span className="text-ink-700">Cookies nécessaires <span className="text-xs text-ink-400">(toujours actifs)</span></span>
                    <input type="checkbox" checked disabled className="h-4 w-4 accent-brand-500" />
                  </label>
                  <label className="flex items-center justify-between text-sm">
                    <span className="text-ink-700">Cookies analytiques</span>
                    <input type="checkbox" checked={analytics} onChange={(e) => setAnalytics(e.target.checked)} className="h-4 w-4 accent-brand-500" />
                  </label>
                  <label className="flex items-center justify-between text-sm">
                    <span className="text-ink-700">Cookies marketing</span>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <button onClick={() => setShow(false)} className="ml-auto rounded-full p-1.5 text-ink-400 hover:bg-ink-100"><X size={16} /></button>
              </div>
            </div>
          </div>
        </motion.div>
