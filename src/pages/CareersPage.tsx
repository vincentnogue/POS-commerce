...
export function CareersPage() {
  ...
  return (
    <FooterPageLayout title="Carrières chez LIYHA GROUP">
      <p className="text-ink-600">Nous construisons l'avenir du commerce africain. Rejoignez une équipe passionnée, pluriculturelle et ambitieuse.</p>

      <section className="mt-8">
        <h2 className="text-xl font-bold text-ink-900">Notre culture</h2>
      ...
      <section className="mt-8">
        <h2 className="text-xl font-bold text-ink-900">Postes ouverts</h2>
        <div className="mt-4 space-y-4">
          {loading && [0, 1].map((i) => (
            <div key={i} className="h-24 animate-pulse rounded-2xl bg-ink-100" />
          ))}
          {!loading && jobs.length === 0 && (
            <p className="text-ink-500">Aucun poste ouvert pour le moment. N'hésitez pas à nous envoyer votre candidature spontanée à <a href="mailto:careers@liyha.group" className="text-brand-600 hover:underline">careers@liyha.group</a>.</p>
          )}
          {!loading && jobs.map((j) => (
            <div key={j.id} className="rounded-2xl border border-ink-200 bg-white p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <h3 className="text-base font-bold text-ink-900">{j.title}</h3>
