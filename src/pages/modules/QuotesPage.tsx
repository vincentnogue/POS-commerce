...
export function QuotesPage() {
  ...
  const save = async () => {
    ...
    // COMPLIANCE FIX: see next_document_number() migration — quotes.length
    // has the exact same duplicate-number risk under concurrent creation
    // or after a deletion that this atomic per-tenant counter closes.
    const { data: seq, error: seqErr } = await supabase.rpc('next_document_number', {
      p_tenant_id: tenant.id, p_doc_type: 'quote', p_year: new Date().getFullYear(),
    });
    if (seqErr || seq == null) { toast('error', seqErr?.message ?? t('quotes.err.numberFailed')); return; }
    const num = `DEV-${new Date().getFullYear()}-${String(seq).padStart(4, '0')}`;
  ...
  const convertToInvoice = async (q: Quote) => {
    ...
    // COMPLIANCE FIX: the previous fix (counting real invoices instead of
    // quotes) was closer but still not atomic — two concurrent conversions,
    // or a conversion racing a manual invoice creation in InvoicesPage,
    // could still read the same count and mint the same number. Both now
    // draw from the same atomic per-tenant 'invoice' sequence.
    const { data: seq, error: seqErr } = await supabase.rpc('next_document_number', {
