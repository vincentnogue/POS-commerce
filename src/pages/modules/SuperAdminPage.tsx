  ...
  if (backendVerified === null) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-sm text-ink-500">Vérification de l'accès…</p>
      </div>
    );
  }
  ...
  if (!backendVerified || !isSuperAdmin || !isPlatformAdmin) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-lg font-semibold text-ink-900">Accès réservé</h2>
  ...
  return (
    <div className="card p-5">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-base font-semibold text-ink-900">{tenants.length} entreprises</h3>
            ...
            {filtered.map((t) => {
              ...
              return (
                <tr key={t.id} className="border-b border-ink-50">
                  ...
                  <td className="py-3 text-ink-500 text-xs">{new Date(t.created_at).toLocaleDateString('fr-FR')}</td>
                  <td className="py-3">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => impersonate(t)} className="rounded-lg p-1.5 text-ink-500 hover:bg-flow-50 hover:text-flow-600" title="Se connecter en tant que"><Eye size={15} /></button>
                      ...
                      <button onClick={() => deleteTenant(t)} className="rounded-lg p-1.5 text-ink-500 hover:bg-error-50 hover:text-error-600" title="Supprimer"><Trash2 size={15} /></button>
                    </div>
                  </td>
                </tr>
              );
      ...
      <Modal open={!!impersonating} onClose={() => setImpersonating(null)} title="Impersonation">
        <p className="text-sm text-ink-600">Connexion en tant que <strong>{impersonating?.name}</strong>… Cette action est journalisée.</p>
      </Modal>
    </div>
  );
}
              ...
              <tbody>
                {filtered.map((m) => (
                  <tr key={m.id} className="border-b border-ink-50">
                    ...
                    <td className="py-3"><p className="text-ink-700">{m.tenants?.name ?? '—'}</p><p className="text-xs text-ink-500">{m.tenants?.city}, {m.tenants?.country_name}</p></td>
...
function SuperSubscriptions() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-5">
        <h3 className="mb-4 text-base font-semibold text-ink-900">Historique des abonnements</h3>
              ...
              <tbody>
                {subs.map((s) => (
                  <tr key={s.id} className="border-b border-ink-50">
                    <td className="py-3"><p className="font-semibold text-ink-900">{s.tenants?.name ?? '—'}</p><p className="text-xs text-ink-500">{s.tenants?.country_name}</p></td>
                    ...
                    <td className="py-3 text-ink-600">{s.billing_cycle}</td>
                    <td className="py-3 text-ink-500 text-xs">{s.current_period_end ? new Date(s.current_period_end).toLocaleDateString('fr-FR') : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
    ...
    <div className="space-y-6">
      <div className="card p-6">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h3 className="text-base font-semibold text-ink-900">Administrateurs de la plateforme (vérifiés côté backend)</h3>
        </div>
        ...
        <div className="space-y-2">
          {platformAdmins.map((pa) => (
            <div key={pa.id} className="flex items-center justify-between rounded-xl border border-ink-100 p-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-ink-900">{pa.email}</p>
...
function SuperMonitoring() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-5">
        <h3 className="mb-4 text-base font-semibold text-ink-900">État des entreprises</h3>
        <div className="space-y-2">
          {tenants.map((t) => (
            <div key={t.id} className="flex items-center justify-between rounded-xl border border-ink-100 p-3">
              <div className="flex items-center gap-3">
                <div className={`h-2.5 w-2.5 rounded-full ${t.status === 'active' ? 'bg-success-500' : 'bg-error-500'}`} />
                <div>
                  <p className="text-sm font-semibold text-ink-900">{t.name}</p>
                  <p className="text-xs text-ink-500">{t.country_name} · {t.currency}</p>
                </div>
              </div>
...
function SuperAudit() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        <button onClick={() => setView('audit')} className={`rounded-full px-4 py-2 text-sm font-semibold ${view === 'audit' ? 'bg-brand-500 text-white' : 'border border-ink-200 bg-white text-ink-700'}`}>Journal d'audit</button>
        <button onClick={() => setView('access')} className={`rounded-full px-4 py-2 text-sm font-semibold ${view === 'access' ? 'bg-brand-500 text-white' : 'border border-ink-200 bg-white text-ink-700'}`}>Tentatives d'accès Super Admin</button>
      </div>
      <div className="card p-5">
        {view === 'audit' ? (
                ...
                <div key={al.id} className={`rounded-xl border p-3 text-sm ${al.authorized ? 'border-success-100 bg-success-50/30' : 'border-error-100 bg-error-50/30'}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold text-ink-900">{al.actor_email ?? 'Inconnu'}</span>
                    </div>
                    <span className="text-xs text-ink-500">{new Date(al.created_at).toLocaleString('fr-FR')}</span>
                  </div>
                  {al.reason && <p className="mt-1 text-xs text-ink-500">{al.reason}</p>}
                </div>
              ))}
            </div>
          )
...
function SuperComms() {
  ...
  return (
    <div className="card max-w-2xl p-6">
      <h3 className="mb-4 text-base font-semibold text-ink-900">Communication globale</h3>
      <p className="mb-4 text-sm text-ink-500">Envoyez une annonce à tous les tenants ou un segment spécifique.</p>
      <div className="space-y-4">
        <Field label="Segment">
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm text-ink-500">{recipientCount} destinataire(s)</p>
