  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        ...
        >
          <p className="text-sm font-semibold uppercase tracking-wide text-brand-600">{t('nav.pricing')}</p>
          <h1 className="mt-2 text-4xl font-extrabold tracking-tight text-ink-900 sm:text-5xl">Un plan pour chaque ambition</h1>
          <p className="mt-4 text-lg text-ink-600">{t('pricing.subtitle')}</p>

          {/* Monthly / Annual toggle */}
          <div className="mt-8 inline-flex items-center gap-1 rounded-full border border-ink-200 bg-ink-50 p-1">
            <button
              onClick={() => setAnnual(false)}
              className={`rounded-full px-5 py-2 text-sm font-semibold transition ${!annual ? 'bg-white text-ink-900 shadow-soft' : 'text-ink-500'}`}
            >
              {t('pricing.monthly')}
            </button>
            <button
              onClick={() => setAnnual(true)}
              className={`flex items-center gap-2 rounded-full px-5 py-2 text-sm font-semibold transition ${annual ? 'bg-white text-ink-900 shadow-soft' : 'text-ink-500'}`}
            >
              {t('pricing.annual')}
          ...
          {PLANS.map((p, i) => {
            ...
            return (
              <motion.div
                ...
                className={`relative flex h-full flex-col rounded-2xl2 border bg-white p-6 ${p.highlight ? 'border-brand-300 shadow-float ring-1 ring-brand-200' : 'border-ink-200'}`}
              >
                {p.popular && (
                ...
                <div className="mt-4 flex items-end gap-1">
                  <span className="text-4xl font-extrabold text-ink-900">${displayPrice}</span>
                  <span className="mb-1 text-sm text-ink-500">{annual ? '/an' : t('pricing.perMonth')} {t('pricing.usd')}</span>
                </div>
                {annual && (
                ...
                <p className="mt-2 text-xs text-ink-500">{p.maxUsers} utilisateurs · {p.maxStores} magasin{p.maxStores > 1 ? 's' : ''}</p>
        ...
        <div className="mt-16 grid gap-6 sm:grid-cols-3">
          {[
            ...
            <div key={f.title} className="rounded-2xl border border-ink-200 bg-brand-50/30 p-5">
        ...
        <div className="mt-12 text-center">
          <Link to="/" className="inline-flex items-center gap-1 text-sm font-semibold text-brand-600 hover:underline">
